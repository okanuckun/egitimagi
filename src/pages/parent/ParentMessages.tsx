...
export default function ParentMessages() {
  ...
  useEffect(() => {
    ...
    const fetchContacts = async () => {
      // Get teacher IDs from children's classes
      ...
      const { data: classes } = await supabase
        .from("classes")
        .select("teacher_id")
        .in("id", students.map((s) => s.class_id));
      ...
      <div className="page-container flex flex-col h-screen">
        ...
        <div className="fixed bottom-0 left-0 right-0 p-3 bg-background border-t">
          <div className="flex gap-2 max-w-lg mx-auto">
            <Input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Mesaj yazın..." onKeyDown={(e) => e.key === "Enter" && sendMessage()} />
