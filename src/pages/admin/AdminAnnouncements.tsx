...
interface Announcement {
  ...
  author_id: string; class_id: string | null; created_at: string;
}
...
export default function AdminAnnouncements() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("announcements").insert({
      title: form.title, content: form.content, author_id: user.id, class_id: null,
    });
    if (error) toast.error("Duyuru oluşturulamadı");
    else { toast.success("Genel duyuru paylaşıldı!"); setOpen(false); setForm({ title: "", content: "" }); fetchData(); }
  };
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-sm mx-auto">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="space-y-2"><Label>İçerik</Label><Textarea value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} required rows={3} /></div>
            ...
            <CardContent className="p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-2">{format(new Date(a.created_at), "d MMM yyyy", { locale: tr })}</p>
                </div>
